...
interface InviteForm { email: string; role: "ORG_ADMIN" | "ORG_MEMBER" }
...
export default function MembersPage() {
  ...
  const invite = useMutation({
    ...
    onSuccess: () => {
      toast.success("Invite sent — they'll receive an email with a link to join.");
  ...
  const changeRole = useMutation({
    ...
    onSuccess: () => { toast.success("Role updated"); qc.invalidateQueries({ queryKey: ["org", "members"] }); },
    onError: (err) => toast.error(getApiError(err)),
  });
    ...
    <div className="space-y-4 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          {data && <p className="text-sm text-muted-foreground">{data.length} member{data.length !== 1 ? "s" : ""}</p>}
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <form onSubmit={handleSubmit((d) => invite.mutate(d))} className="space-y-4 mt-2">
              ...
              <div className="space-y-1">
                <Label>Role</Label>
                <Select defaultValue="ORG_MEMBER" onValueChange={(v) => setValue("role", v as InviteForm["role"])}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="ORG_MEMBER">Member — can view org info and their profile</SelectItem>
                    <SelectItem value="ORG_ADMIN">Admin — full access to billing, members, subscription</SelectItem>
                  </SelectContent>
                </Select>
              </div>
                ...
                {data?.map((m) => (
                  <TableRow key={m.id}>
                    ...
                    <TableCell>
                      <Select
                        defaultValue={m.role}
                        onValueChange={(v) => changeRole.mutate({ userId: m.id, role: v })}
                      >
